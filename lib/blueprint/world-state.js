...
const MAX_BEAT_LORE_KEYS = 6;
let loreEntryCounter = 0;
...
export function selectRelevantLoreEntries(blueprintState, scene, options = {}) {
    ...
    const maxEntries = Math.max(1, parseInt(options.maxEntries ?? DEFAULT_MAX_LORE_ENTRIES, 10));
...
export function buildLoreInjection(loreEntries = [], linkedLorebooks = [], options = {}) {
    const maxContentLength = Math.max(80, parseInt(options.maxContentLength ?? DEFAULT_MAX_LORE_CONTENT, 10));
    const itemXml = loreEntries.map(entry => {
        ...
        const clipped = clipAtWordBoundary(text, maxContentLength);
...
export function addLoreEntryFromBeatCompletion(blueprintState, { sceneIndex, beatIndex, scene, beatTitle }) {
    ...
    const entry = {
        uid: generateEntryUid(),
        key: buildLoreKeys(scene, `${sceneTitle} ${title}`).slice(0, MAX_BEAT_LORE_KEYS),
...
function addLoreEntryIfMissing(blueprintState, entry) {
    ...
    const embeddedLorebook = ensureEmbeddedLorebook(blueprintState.blueprint);
    embeddedLorebook.entries ??= [];

    const signature = entry?.extensions?.signature;
    const exists = signature
        ? embeddedLorebook.entries.some(e => e?.extensions?.signature === signature)
        : embeddedLorebook.entries.some(e => e.comment === entry.comment && e.content === entry.content);

    if (exists) return false;

    embeddedLorebook.entries.push(entry);
    return true;
}
...
function inferLocation(scene, summary) {
    ...
    const match = text.match(/\b(?:at|in|inside|within)\s+([\p{L}\p{N}][\p{L}\p{N}' \-]{2,40})/iu);
    return match ? match[1].trim() : null;
}

function generateEntryUid() {
    loreEntryCounter += 1;
    return (Date.now() * 1000) + loreEntryCounter;
}

function clipAtWordBoundary(text, maxLength) {
    if (text.length <= maxLength) return text;
    const softClip = text.slice(0, maxLength);
    const breakIndex = Math.max(
        softClip.lastIndexOf(' '),
        softClip.lastIndexOf('.'),
        softClip.lastIndexOf(','),
        softClip.lastIndexOf(';')
    );
    const clipped = breakIndex > 40 ? softClip.slice(0, breakIndex) : softClip;
    return `${clipped.trim()}...`;
}
