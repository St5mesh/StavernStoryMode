...
import { validateScenePlanOutput, validateSceneBatchOutput } from './validation.js';
 ...
 */
function parseStagedResponse(subPhase, rawText, maxTokens, expectedIndices) {
    ...
    try {
        ...
        validator(data, expectedIndices);
        return data;
    } catch (parseError) {
        const preview = rawText.substring(0, 120).replace(/\s+/g, ' ');
        console.error(`[Story Mode] Staged ${subPhase} parse error — output preview: "${preview}..." — ${parseError.message}`);
        throw createError(`Staged ${subPhase}: Failed to parse JSON - ${parseError.message}`);
    }
}
...
async function executeScenePlan(request, storyType, authorStyle, partialBlueprint, profileId, callbacks) {
    const { planMaxTokens } = STAGED_SCENE_CONFIG;

    const data = await executeWithRetry(
        'Generating scene outlines (Phase 3a)',
        async (attempt, lastError) => {
            // Double token budget on each retry: attempt 0=1x, 1=2x, 2=4x
            const adjustedTokens = planMaxTokens * Math.pow(2, attempt);
            if (attempt > 0) {
                console.log(`[Story Mode] Retry ${attempt}: increasing token budget to ${adjustedTokens}`);
            }
...
async function executeSceneBatch(
    ...
    const { batchMaxTokens, batchSize } = STAGED_SCENE_CONFIG;
    ...
    const data = await executeWithRetry(
        ...
        async (attempt, lastError) => {
            // Double token budget on each retry: attempt 0=1x, 1=2x, 2=4x
            const adjustedTokens = batchMaxTokens * Math.pow(2, attempt);
            if (attempt > 0) {
                console.log(`[Story Mode] Batch ${batchIndex + 1} retry ${attempt}: increasing token budget to ${adjustedTokens}`);
            }
