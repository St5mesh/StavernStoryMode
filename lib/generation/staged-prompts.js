...
function buildPersonaSection(personaData) {
    ...
    const personaDataStr = personaData.map(formatPersona).join('\n---\n');
    return `---\n### PERSONA DATA\n\n${personaDataStr}\n\n---`;
}
...
export function compressCompletedScenes(completedScenes, currentBatchStart, batchSize) {
    ...
    return {
        summaryText: truncateContextSection(olderScenes.length
            ? olderScenes.map(formatSceneSummary).join('\n')
            : 'No earlier scenes (this is the first or second batch).', STAGED_SCENE_CONFIG.completedSummaryMaxChars),
        previousBatchJson: truncateContextSection(prevBatchScenes.length
            ? JSON.stringify(prevBatchScenes.map(compactSceneForContinuity), null, 2)
            : 'No previous batch (this is the first batch).', STAGED_SCENE_CONFIG.previousBatchContextMaxChars),
    };
}
